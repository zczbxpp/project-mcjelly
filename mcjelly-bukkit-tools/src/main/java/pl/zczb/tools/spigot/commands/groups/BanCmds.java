...
public class BanCmds {
    ...
    public void mutePlayer(
        ...
        Controller.getInstance().getRedis().publish("CH|" + Tools.getSectorConfig().getCurrentSector().getSectorType().toString().toLowerCase(), new BanPacket("ban", targetUser.getNick(), reason, sender.getName(), muteUntilTimestamp));

        sender.sendMessage(GlobalHelper.fixColor("&8[&#20EA00&l!&8] &#6FFF58Pomyślnie zbanowano gracza &2" + targetUser.getNick() + " &ana czas &2" + muteDurationDisplay + " &az powodem &2" + reason));
    }

    @Execute(name = "unban")
    @Permission({"zczb.admin"})
    public void unmute(@Context Player sender, @Arg("gracz") UserDataModel targetUser) {
        Controller.getInstance().getRedis().publish("CH|" + Tools.getSectorConfig().getCurrentSector().getSectorType().toString().toLowerCase(), new BanPacket("unban", targetUser.getNick(), null, sender.getName(), null));
        sender.sendMessage(GlobalHelper.fixColor("&8[&#20EA00&l!&8] &#6FFF58Pomyślnie odbanowano gracza &2" + targetUser.getNick()));
    }
