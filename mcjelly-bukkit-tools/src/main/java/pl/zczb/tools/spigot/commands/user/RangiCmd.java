...
@Command(name = "rangi", aliases = {"vip", "svip", "mvip", "donator", "media"})
public class RangiCmd {
    @Execute
    public void onChannel(@Context CommandSender sender) {
        ...
        Gui gui = Gui.gui()
                .title(Component.text(GlobalHelper.fixColor("&8Rangi")))
                .rows(5)

                .disableAllInteractions()
                .create();

        for (Help warp : Tools.getRangiConfig().values()) {
...
//        gui.setItem(21, svip);
//        gui.setItem(22, mvip);
//        gui.setItem(23, donator);
//        gui.setItem(24, media);
